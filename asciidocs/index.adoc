...
Now the filter functionality will be created. All events and all fields of the objects will be filtered.
So it could be in some way tedious.

.src/overview/entity/Filter.js
[source,javascript]
----
...
image:less035-project-structure-test.png[]

Copy from https://mochajs.org/#running-mocha-in-the-browser the code into `test.html`

.test.html (https://mochajs.org/#running-mocha-in-the-browser[source])
[source,html]
----
<!DOCTYPE html>
<html lang="en">
...
----
browser-sync test -f "src" "test" --index="test.html"  -b "google chrome"
----


image:less035-first-mocha-site.png[]
...
image:less035-first-test.png[]

=== An Empty Function Test

First we download the libraries into a local lib-folder.

----
mkdir test/lib
cd test/lib
curl -L -O https://unpkg.com/chai/chai.js
curl -L -O https://unpkg.com/mocha/mocha.js
curl -L -O https://unpkg.com/mocha/mocha.css
----
...
image:less036-result-in-browser.png[]


=== Unit Testing matchesCriteria

The test is working with empty filter, but every other filter will break the test.

==== matchesCriteria_true_emptyEvent
...
describe('#overview.entity.Filter', function () {
    it('matchesCriteria_true_emptyEvent', function () {
        assert.isTrue(matchesCriteria({},'abc')); // <.>
    });
})
...
----

image:less037-broken-test.png[]

==== matchesCriteria_true_webEvent_and_emptyFilter
...
image:less037-two-tests.png[]



==== matchesCriteria_true_webEvent_and_matchingFilter
...
image:less037-three-tests.png[]


==== matchesCriteria_true_webEvent_and_partiallyMatchingFilter
...
const matchesCriteria = (event, filter) => {
    ...
    const values = Object.values(event);
    console.log(values);
    const size = values.filter(value => value.includes(filter)).length;
    return size >= 1;
};
export default matchesCriteria;
----

image:less037-four-tests.png[]

==== matchesCriteria_true_webEvent_and_partiallyMatchingFilter_ignoreCase
